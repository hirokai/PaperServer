  ...
  where
    ...
    status (Reference _ _ (Just cit) _ _) = do
        res <- case doi of
                  Just d -> do
                    email <- requireAuthId
                    runDB $ selectListUser email [PaperDoi ==. d] []
                  Nothing -> return []
        return $ case (res::[Entity Paper]) of
                ((Entity id p):_) -> Downloaded
                _ -> Unavailable -- Stub
...
insertUserH email (History pid HACreate time _)
  = insert (History pid HACreate time (User email Nothing Nothing))
